...
if(isset($_SESSION['ricerca'])){
    ...
    foreach($qResult_route as $ris){
        ...
        $qResult_price[0] = number_format(($qResult_price[0] - $qResult_price[0] * $discount_code / 100) * $nPasseggeri, 2);
        ...
        foreach ($qResult_departure as $dt) {
            $departure_time_station = (new DateTime($dt))->add(getDateInterval($qResult_time[0]));
            ...
            $train_id = $connessione->getDataArray("select route_schedule.train_id from route_schedule where route_schedule.route_id=$ris 
                and route_schedule.departure_time = '".$dt."'");
            ...
            $ticket = $ticket . '<article id="'.$counter.'" class="ticket js-ticket">
                        ...
                        <a href="#'.(($counter_departures==$total_departures - 1 && $counter_routes==$total_routes - 1) ? ($counter + 1) : 0).
                        ...
                        <button class="submit submit--ticket js-submit" data-firstClass="'.$qResult_price[0].'" data-secondClass="'
